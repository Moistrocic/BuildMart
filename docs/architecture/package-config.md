...
- 键：
  ...
  - `flyDigSpeedRestore`（boolean，默认 true）——飞行挖掘速度恢复开关（开启=飞行中挖掘与地面一致；
    关闭=保留原版空中挖掘惩罚 `f / 5.0F`）。
  - `shopSellLog`（boolean，默认 false）——商店出售结算日志开关（每店每 60 秒一条，默认关闭防刷屏）。

...
## 4. 使用 IntelliJ IDEA 开发

- 本项目使用 IntelliJ IDEA（IDEA）作为主要开发 IDE，提交与构建方式需保持对 IDEA 的兼容。
- 使用 Gradle Wrapper（`gradlew` / `gradlew.bat`）和 Loom 提供的任务（如 `genSources`、`runClient`、`runServer`、`build`）进行开发与运行。
- 保持项目可被 IDEA 直接导入和运行；新增依赖或修改构建脚本后，在 IDEA 中刷新/重新导入 Gradle 项目并确认可正常构建。
- `.idea/`、`*.iml` 等 IDE 个人配置文件已被 `.gitignore` 忽略，不要提交这些文件，也不要提交本机专用路径或个人运行配置。
- 生成或修改代码后，保持源码结构符合 Loom 的 `src/main`（通用/服务端）与 `src/client`（仅客户端）分离约定。

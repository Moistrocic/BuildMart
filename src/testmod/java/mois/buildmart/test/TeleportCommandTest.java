...
public final class TeleportCommandTest {
	...
	public void homeLifecycle(GameTestHelper helper) {
		...
		// 开放家功能（默认上限 0）
		admin.execute("/bm config home.max 2").expectMessage("已设置为");
		...
		admin.execute("/bm config home.max 0");
		helper.succeed();
	}
	...
	public void teleportRequestErrorPaths(GameTestHelper helper) {
		...
		// 开放后：目标不在线
		admin.execute("/bm config tpa.enabled true").expectMessage("已设置为");
		...
		admin.execute("/bm config tpa.enabled false");
		helper.succeed();
	}
	...
	public void backErrorPathAndConsoleOnly(GameTestHelper helper) {
		TestPlayer admin = TestPlayer.admin(helper);
		admin.execute("/bm config back.enabled true").expectMessage("已设置为");
		TestPlayer player = TestPlayer.player(helper);
		player.execute("/back").expectMessage("没有可返回的死亡点");
		admin.execute("/bm config back.enabled false");

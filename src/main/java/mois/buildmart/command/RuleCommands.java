...
/**
 * 部分规则调整指令（受 /config partialRuleAdjust 控制，配置为 true 时**普通玩家**也可用；
...
public final class RuleCommands {
	...
	/**
	 * 部分规则调整权限：配置开启（/config partialRuleAdjust true）或等级 2 及以上管理员。
	 * 动态读取配置，因此 /config 热改即时生效（含收回权限）。
	 */
	private static boolean allowed(CommandSourceStack source) {
		return EconomyConfig.partialRuleAdjust() || GAMEMASTER.test(source);
	}
	...
	private static void relaxVanillaPermission(CommandDispatcher<CommandSourceStack> dispatcher, String name) {
		...
		try {
			...
			BuildMart.LOGGER.info("/{} 指令权限已挂载部分规则调整开关（/config partialRuleAdjust）", name);
